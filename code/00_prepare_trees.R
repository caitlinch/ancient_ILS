...
#### 15. Find smallest coalescent difference in ASTRAL trees ####
# Open ASTRAL species tree
t_files <- list.files(tree_output_dir)
a_tree_file <- paste0(tree_output_dir, grep("ASTRAL_tree.tre", t_files, value = T))
a_tree <- read.tree(a_tree_file)
# Root ASTRAL tree at the Choanoflagellates
r_tree <- root(a_tree, outgroup = c("Salpingoeca_pyxidium", "Monosiga_ovata", "Acanthoeca_sp", "Salpingoeca_rosetta", "Monosiga_brevicolis"), resolve.root = T)
# Add terminal branches
r_tree$edge.length[which(is.na(r_tree$edge.length))] <- 0.5
# Make the ASTRAL tree ultrametric by extending terminal branches
u_tree <- force.ultrametric(r_tree, method = "extend")
# Identify the coalescent intervals for the (rooted, ultrametric) ASTRAL tree
coal_ints <- coalescent.intervals(u_tree)
# Plot a histogram of the coalescent intervals
hist(coal_ints$interval.length)
# Identify the minimum coalescent interval (excluding 0)
coal_interval_length <- coal_ints$interval.length
coal_interval_length <- coal_interval_length[which(coal_interval_length != 0)]
print(summary(coal_interval_length))
# Minimum coalescent interval is 0.004921 for the ASTRAL tree (when the ASTRAL tree is ultrametric and rooted)
#   Scientific format: minimum coalescent interval = 4.921 * 1e-03
# Keep magnitude of minimum coalescent interval identical for simulation parameter - use 0.001 (1e-03)
