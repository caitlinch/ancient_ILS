...
extract.C60.parameters <- function(iqtree_file){
  ...
  if (file.exists(iqtree_file) == TRUE){
    ...
    bfm_ind <- grep("Best-fit model according to BIC", iq_lines)
    bfm_str <- gsub(" ", "", strsplit(iq_lines[bfm_ind], ":")[[1]][2])
    
    ## Extract mixture model of substitution
    mm_ind <- grep("Mixture model of substitution", iq_lines)
    mm_str <- gsub(" ", "", strsplit(iq_lines[mm_ind], ":")[[1]][2])
    
    ## Extract table with rates and weights for C60 model
    # Extract table from iqtree
    ...
    # Paste MIX argument together
    mixture_model_str <- paste0("MIX{", paste(table_df$component_op, collapse = ","), "}")
    
    ## Extract aa freqs
    aa_ind              <- grep("State frequencies", iq_lines)
    aa_emp_counts_check <- grep("empirical counts from alignment", iq_lines)
    pi_A_check          <- grep("pi\\(A\\)", iq_lines)
    if (aa_ind == aa_emp_counts_check | identical(pi_A_check, integer(0)) == FALSE){
    ...
    ## Extract RHAS rates and weights
    rhas_ind      <- grep("Model of rate heterogeneity", iq_lines)
    rhas_r        <- as.numeric(str_extract(iq_lines[rhas_ind], "(\\d)+"))
    rhas_wr_split <- strsplit(grep("Site proportion and rates", iq_lines, value = T), ":")[[1]][2]
    rhas_wr_raw   <- paste(strsplit(rhas_wr_raw, "\\) \\(")[[1]], collapse = ",")
    rhas_wr       <- gsub("\\)", "", gsub("\\(", "", gsub(" ", "", rhas_wr_raw)))
    rhas_op       <- rhas_wr
    
    ## Assemble full model
    full_model <- paste0(mixture_model_str, "+F{", aa_op,"}+R{", rhas_op, "}")
