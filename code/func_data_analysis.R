...
process.CTEN.tree <- function(CTEN_cf_tree, CTEN_cf_stat, dataset_info, bilat_taxa, cnid_taxa, cten_taxa, plac_taxa, pori_taxa, outg_taxa){
  ...
  if (length(pori_taxa) > 1 & length(cten_taxa) > 1){
    ...
    # Extract scf
    aoa_end   <- getMRCA(aoa_tree, tip = c(bilat_taxa, cnid_taxa, pori_taxa))
    aoa_check <- aoa_tree$edge[which(aoa_tree$edge[,2] == aoa_end), 1]
    aoa_branch  <- intersect(which(aoa_tree$edge[ , 1] %in% aoa_check), which(aoa_tree$edge[  ,2] == aoa_end)) 
    if (length(aoa_branch) > 1){
      aoa_branch <- min(aoa_branch)
    }
    aoa_node    <- aoa_tree$node.label[aoa_branch]
    if (aoa_node == "Root" | nchar(aoa_node) == 0){
      aoa_ntaxa    <- length(c(bilat_taxa, cnid_taxa, cten_taxa))
      aoa_vector   <- as.character(c(dataset_info, "CTEN_sister", "ALL_ANIMALS", aa_ntaxa, aoa_node, rep(NA, 9)))
    } else {
      aoa_bs      <- as.numeric(strsplit(aoa_node, "/")[[1]][1])
      aoa_scf     <- as.numeric(strsplit(aoa_node, "/")[[1]][2])
  ...
  if (length(cten_taxa) > 1){
    ...
    c_ntaxa   <- length(cten_taxa)
    c_vector  <- as.character(c(dataset_info, "CTEN_sister", "CTEN_CLADE", c_ntaxa, rep(NA, 10) ))
  }
  ...
  if (length(pori_taxa) > 1){
    ...
    p_ntaxa   <- length(pori_taxa)
    p_vector  <- as.character(c(dataset_info, "CTEN_sister", "PORI_CLADE", p_ntaxa, rep(NA, 10) ))
  }
  # Assemble rows into output table
  cten_op_table <- as.data.frame(rbind(aa_vector, c_vector, aoa_vector, p_vector))
  colnames(cten_op_table) <- c("dataset", "matrix", "dataset_id", "gene_name", "gene_id", "tree_topology", "branch_to_clade", "num_taxa", 
...
process.PORI.tree <- function(PORI_cf_tree, PORI_cf_stat, dataset_info, bilat_taxa, cnid_taxa, cten_taxa, plac_taxa, pori_taxa, outg_taxa){
  ...
  if (length(pori_taxa) > 1){
    ...
    p_ntaxa   <- length(pori_taxa)
    p_vector  <- as.character(c(dataset_info, "PORI_sister", "PORI_CLADE", p_ntaxa, rep(NA, 10) ))
  }
  ...
  if (length(cten_taxa) > 1){
    ...
    c_ntaxa   <- length(cten_taxa)
    c_vector  <- as.character(c(dataset_info, "PORI_sister", "CTEN_CLADE", c_ntaxa, rep(NA, 10) ))
  }
  # Assemble rows into output table
  pori_op_table <- as.data.frame(rbind(aa_vector, p_vector, aoa_vector, c_vector))
  colnames(pori_op_table) <- c("dataset", "matrix", "dataset_id", "gene_name", "gene_id", "tree_topology", "branch_to_clade", "num_taxa", 
                               "ID", "sCF", "sCF_N", "sDF1", "sDF1_N", "sDF2", "sDF2_N", "sN", "ultafast_bootstrap", "branch_length")
...
process.CTEN_PORI.tree <- function(CTEN_PORI_cf_tree, CTEN_PORI_cf_stat, dataset_info, bilat_taxa, cnid_taxa, cten_taxa, plac_taxa, pori_taxa, outg_taxa){
  ...
  if (length(outg_taxa) > 1){
    ...
    aa_tab_extract  <- scf_tab[aa_row_to_extract, ]
    aa_vector  <- as.character(c(dataset_info, "CTEN_PORI_sister", "ALL_ANIMALS", aa_ntaxa, aa_tab_extract))
  } else if (length(outg_taxa) == 0){
    aa_ntaxa   <- length(c(bilat_taxa, cnid_taxa, cten_taxa, pori_taxa))
    aa_vector  <- as.character(c(dataset_info, "CTEN_PORI_sister", "ALL_ANIMALS", aa_ntaxa, aa_node, rep(NA, 9) ))
  } else if (length(outg_taxa) == 1){
    # Root at outgroup
    aa_tree <- root(gene_tree_drop, outgroup = outg_taxa, resolve.root = T)
    # Extract CTEN tree, ALL ANIMALS branch (AA)
  ...
  if (length(bilat_taxa) > 0 & length(cnid_taxa) > 0){
    ...
    if (aoa_node == "Root" | nchar(aoa_node) == 0){
      aoa_ntaxa    <- length(c(bilat_taxa, cnid_taxa, cten_taxa, pori_taxa))
      aoa_vector   <- as.character(c(dataset_info, "CTEN_PORI_sister", "ALL_OTHER_ANIMALS", aoa_ntaxa, aoa_node, rep(NA, 9)))
    } else {
      aoa_bs      <- as.numeric(strsplit(aoa_node, "/")[[1]][1])
      aoa_scf     <- as.numeric(strsplit(aoa_node, "/")[[1]][2])
      aoa_length  <- as.numeric(gene_tree$edge.length[aoa_branch])
  ...
  } else if (length(bilat_taxa) == 1 | length(cnid_taxa) == 1){
    # Root at outgroup
    aoa_tree <- root(gene_tree_drop, outgroup = outg_taxa, resolve.root = T)
    # Extract scf
    aoa_start   <- getMRCA(aoa_tree, tip = c(bilat_taxa, cnid_taxa))
  ...
  if (length(cten_taxa) > 1){
    ...
    c_ntaxa   <- length(cten_taxa)
    c_vector  <- as.character(c(dataset_info, "CTEN_PORI_sister", "CTEN_CLADE", c_ntaxa, rep(NA, 10) ))
  }
  # Extract CTEN tree, PORI branch (P)
  if (length(pori_taxa) > 1){
    p_start   <- getMRCA(gene_tree, tip = pori_taxa)
    ...
    p_ntaxa   <- length(pori_taxa)
    p_vector  <- as.character(c(dataset_info, "CTEN_PORI_sister", "PORI_CLADE", p_ntaxa, rep(NA, 10) ))
  }
  # Assemble rows into output table
  cten_pori_op_table <- as.data.frame(rbind(aa_vector, cp_vector, c_vector, p_vector, aoa_vector))
  colnames(cten_pori_op_table) <- c("dataset", "matrix", "dataset_id", "gene_name", "gene_id", "tree_topology", "branch_to_clade", "num_taxa", 
                                    "ID", "sCF", "sCF_N", "sDF1", "sDF1_N", "sDF2", "sDF2_N", "sN", "ultafast_bootstrap", "branch_length")
